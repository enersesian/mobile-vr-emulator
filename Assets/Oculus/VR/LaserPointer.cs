...
//Moved script in Oculus.VR folder so its in the same Project as OVRHeassetEmulator. Fixed this situation later.
public class LaserPointer : OVRCursor
{
    ...
    public override void SetCursorRay(Transform t)
    {
        ...
        _endPoint = _startPoint + maxLength * (transform.rotation * Vector3.forward);
        _forward = t.forward;
        _hitTarget = false;
    }
    ...
    private void LateUpdate()
    {
        ...
        if (_hitTarget)
        {
            //_endPoint = _startPoint + maxLength * (transform.localRotation * Vector3.forward);
